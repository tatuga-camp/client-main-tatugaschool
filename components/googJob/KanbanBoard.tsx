...
export default function KanbanBoard({ data }: KanbanBoardProps) {
  ...
  function handleDragEnd(event: any) {
    ...
    setColumns(prevColumns => {
      ...
      // หา task ที่กำลังลาก
      const draggedTask = newColumns[activeColumnIndex].tasks
      ...
      newColumns[activeColumnIndex].tasks =
        newColumns[activeColumnIndex].tasks.filter(task => task.id !== activeId);
      ...
      if (isOverColumn) {
        ...
        const overTaskIndex = newColumns[overColumnIndex].tasks
          ...
          <div className="flex gap-4 overflow-x-scroll pb-4">
            {columns.map((column) => (
              <SortableColumn
                ...
                onEditTitle={(newTitle) => handleEditColumnTitle(column.id, newTitle)}
                type="column"
              />
            ))}
          </div>
