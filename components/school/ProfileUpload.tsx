...
import { getSignedURLTeacherService, RequestUpdateSchoolService, UploadSignURLService } from '@/services';
import { UseMutationResult } from '@tanstack/react-query';

const ProfileUpload: React.FC<{ school: School, updateSchool: UseMutationResult<School, Error, RequestUpdateSchoolService, unknown> }> = ({ school, updateSchool }) => {
    ...
    const handleUpload = async (url: string) => {
        if (!url) return;
        await updateSchool.mutateAsync({ query: { schoolId: school.id }, body: { logo: url } });
    }
        ...
        <div className="w-full p-6 bg-white rounded-xl text-center">
            <div className="relative w-48 h-48 mx-auto rounded-full overflow-hidden border-2 border-dashed border-gray-300">
                <Image
                    ...
                    width={200}
                    height={200}
                    className={`object-cover ${previewUrl || school.logo ? "" : "grayscale"}`}
                />
