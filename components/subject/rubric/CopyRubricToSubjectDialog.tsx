...
function CopyRubricToSubjectDialog({
  rubric,
  currentSubjectId,
  schoolId,
  educationYear,
  ...
  const [selected, setSelected] = useState<SubjectWithMeta | null>(null);
  ...
  const handleCopy = async () => {
    if (!selected) return;
    try {
      const tree = await GetRubricByIdService({ rubricId: rubric.id });
      await createRubric.mutateAsync(buildCopyRequest(tree, selected.id));
      toast.current?.show({
        severity: "success",
      ...
      <div className="flex max-h-[90vh] w-[95vw] max-w-4xl flex-col gap-4 overflow-hidden rounded-2xl border bg-background-color p-5 font-Anuphan">
        <header className="flex flex-col gap-1 border-b pb-3">
          <h2 className="text-lg font-semibold">
            {rubricLanguage.copyToSubject(language.data ?? "en")}
          </h2>
          <p className="truncate text-sm text-gray-500">{rubric.title}</p>
        </header>
        ...
        <footer className="flex items-center justify-end gap-2 border-t pt-3">
          ...
          <button
            type="button"
            disabled={!selected || createRubric.isPending}
            onClick={handleCopy}
...
export default CopyRubricToSubjectDialog;
